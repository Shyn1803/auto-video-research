...
| 5-3 | ✅ done | 3 | feat/5-3-assetpicker | All 6 steps done. Backend: `GET /assets/stock-status`, `GET /assets/search`, `POST /assets/upload`, `POST /assets/fetch-stock` (`app/api/assets.py`, `app/services/asset_service.py`) + `assets` table/migration 012 + Pexels/Pixabay/Unsplash/minio adapters (none existed before this task — see project-memory Open Questions) + `app/core/asset_allowlist.py` shared by SSRF fetch-stock check and new AC4 raw-URL-reject-422 on `PUT scenes/{id}`. Fixed pre-existing bug: `ProviderRouter.get_chain("asset_stock")` derived `ASSET_STOCK_CHAIN` but config field is `asset_chain`/`ASSET_CHAIN` — special-cased in router.py. Frontend: `AssetPicker.tsx` modal shell (focus-trap+ESC) + `ProjectAssetsTab`/`UploadTab`/`StockSearchTab`, `lib/api/assets.ts`, `/dev/asset-picker` harness page (real SceneFormPanel wiring deliberately left to whoever next touches that file — 5-2/5-4 concurrently in-progress). All 5 ACs test-covered (backend: 341 passed/20 failed pre-existing-unrelated, baseline was 334/24; frontend: 51/51 passed) + Playwright e2e (asset-picker.spec.ts) run against a live dev server, confirmed no direct pexels.com data call. Also added missing `frontend/package.json` `typecheck`/`test:e2e` scripts + ran first-ever `npm install` in this worktree, surfacing 15 pre-existing unrelated tsc errors (logged, not fixed — see project-memory). |
