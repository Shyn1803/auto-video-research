...
| 4-5 | 🔵 in-progress | 7 | feat/4-5-node-write-outline-script | Steps 1-7 done: warnings[] content contract, BR-1 FAIL-claim context filter, outline node (7-section+citation), script node + number-set subset validator (pure fn, fixed a Unicode tokenizer bug + a homograph false-positive), voice_over symbol-leak (BR-2), title-length guard (BR-3), WARN-disclosure validator (BR-4, fixed a keyword-overlap false-positive). 30/30 write-node unit tests green. Remaining: Step 8 (approve endpoints + PUT manual edit + AC5 lineage) and Step 9 (AC coverage sweep). |
