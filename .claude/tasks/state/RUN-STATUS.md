...
| 3-3 | 🔵 in-progress | 0 | feat/3-3-llm-adapter-async-await-fix | Merged to main 2026-07-17 (later): adapter code (llm+tts) was already on main from a prior session; fixed MockTts async/override bugs (test_mock_adapter.py) and missing `await adapter.available()` in test_demo/gemini/groq/openrouter.py. Still red: test_gemini.py::TestBuildGeminiBody (3, real schema-building bug) and test_embedding.py (5, bge_m3 env gap + cosine-similarity/precision issues) — flagged for a dedicated follow-up, not fixed this pass. |
