...
| 4-7 | 🔴 blocked (Step 7 only) | 7 | feat/4-7-dieu-khien-run-huy-ngam-resume | Steps 1-6+8 done (backend fully working: run-status edge, cancel_requested_at migration 012, cancel endpoint, usage/version preservation, run.cancelled event + event-catalog/api-spec update, resume-after-cancel reopens the cancelled run's own row so LangGraph's thread_id-keyed checkpoint actually continues, 20x race test). Step 7 (FE Cancel-button/RunningState wiring, AC4) blocked: task 5-8 (RunningState component) is still `backlog`, nothing in frontend/src to wire into. See state/4-7.json decisions + project-memory.md Open Questions. |
