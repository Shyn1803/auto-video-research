...
| 4-5 | ✅ done | 9 | feat/4-5-node-write-outline-script | All 9 steps done: warnings[] content contract (api-spec §3 updated), BR-1 FAIL-claim context filter, outline node (7-section+citation), script node + number-set subset validator (pure fn; fixed a Unicode-diacritic tokenizer bug, a "năm"/"5" homograph false-positive, and a [source_id]-digit-leak bug caught by the AC1 integration test), voice_over symbol-leak (BR-2), title-length guard (BR-3), WARN-disclosure validator (BR-4, fixed a keyword-overlap false-positive), first-ever `POST .../steps/{step}/approve` + `PUT .../versions/current` implementation (new `step_approvals` table/migration 012, scoped to outline/script), 5 AC-mapped integration tests. 58/58 task-4-5 tests green; full backend suite 403 passed / 31 pre-existing unrelated failures (TTS adapter + event-envelope signature drift, not touched by this task). Node functions follow 4-3/4-4 precedent: self-contained, tested, not yet wired into graph.py's NODE_FNS (see patterns/langgraph-pipeline-node.md). |
