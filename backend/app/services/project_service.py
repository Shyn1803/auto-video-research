...
``NEXT_ACTION_MAP`` / ``LIFECYCLE_GROUPS`` live in ``app.schemas.project`` — single
source shared with the API layer's response building.
"""
...
class ProjectService:
    ...
    async def create(
        ...
        *,
        ...
        entry_point = "script" if script_text else "research"
        proj = Project(
            name=name,
            topic=topic,
            mode=mode,
        ...
        # BR-4: if script was provided, persist it as script v1 with created_by=user
        if script_text is not None:
            from app.services.versioning_service import VersioningService
            vs = VersioningService(self._db)
            await vs.create(
                project_id=proj.id,
                step="script",
                content={"text": script_text},
                actor="user",
            )
    ...
    async def delete(self, project: Project, actor: str = "user") -> None:
        ...
        Raises TransitionError (mapped to 409 by the router) otherwise — the
        caller is pointed at archive instead.
        """
