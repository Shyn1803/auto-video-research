...
AI Prompt → Research → [AI] Semantic Storyboard        ← AI dừng ở đây
                      ...
                      ④ Constraint Resolver            ← v1: preset flex theo class; v2: solver tổng quát
                              ▼
                      ⑤ Responsive Solver              ← format + platform profile từ cùng tree, không gọi lại AI
                              ▼
...
Đây là **schema input riêng, strict**: Pydantic phải `extra="forbid"` (và JSON Schema/Zod tương đương) để reject mọi field ngoài danh sách, đặc biệt `layout`, `layout_override`, `position`, `color`, `font`, `animation`, `motion_plan`, `transition`, `camera`, `constraints`. Không parse trực tiếp output LLM vào Scene JSON resolved.
...
Boundary bắt buộc: `SemanticStoryboard` → validate strict → `SceneTree` → Layout Engine → `ResolvedScene`. Chỉ hàm resolve nội bộ của engine có thể khởi tạo `ResolvedScene`; API nhận sửa từ editor phải map về semantic tree hoặc field override được phép, rồi resolve lại. Test contract phải chứng minh LLM payload chứa một field bị cấm bị reject trước khi chạm resolver.
...
`platform_profile` là input deterministic thứ hai, tách khỏi `format`: `generic`, `tiktok`, `facebook_reels`, `youtube_shorts`, `youtube_video`. Nó chỉ chọn token safe-area/caption/watermark và validator output; không phải input AI, không đổi layout class và không gọi lại LLM. Một profile phải tương thích format (ví dụ `tiktok`/`facebook_reels`/`youtube_shorts` yêu cầu 9:16; `youtube_video` yêu cầu 16:9).
