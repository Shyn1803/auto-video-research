...
| `run.cancelled` | `{project_id, run_id, step}` — Task 4-7: published once a cancel request (`POST /projects/{id}/runs/{run_id}/cancel`) is honored, i.e. after the in-flight node genuinely stops (never mid-write) |
