...
| PUT 🅞 | `.../versions/current` | (task 4-5) Sửa tay bản hiện hành: `{content}` → tạo version mới (insert-only, vẫn tuân BR-1) với `created_by="user"`, `parent_version` = version vừa sửa. Dùng cho outline/script (và tái dùng được cho step khác nếu cần) khi user edit trực tiếp thay vì regenerate. |
| POST 🅞 | `/projects/{id}/steps/{step}/approve` | (task 4-5, đầu tiên hiện thực hoá endpoint đã khai ở §2) Duyệt version hiện hành của step. Scope task 4-5: chỉ `outline`/`script` — 2 sub-step duyệt riêng (outline duyệt xong mới được chạy script). Ghi nhận qua bảng `step_approvals` (tách khỏi `content`, cùng lý do với `scene_approvals` — approval là workflow state, không phải nội dung version). 409 nếu version hiện hành đã stale hoặc không tồn tại. |
...
**Contract change note (task 4-5):** thêm chuẩn hoá `warnings: [{type, detail}]` bên trong `content` JSONB của `outline`/`script` version (xem `app/schemas/step_version.py::ContentWarning` — BR-2/BR-3/BR-4 và số-liệu-lệch của Step 4 đều phát warning qua đúng 1 shape này, không phải 4 shape khác nhau theo từng rule). `script` content thêm field `source_outline_version: int | null` — con trỏ lineage cross-step (script được sinh từ outline version nào) phục vụ AC5; đây là field trong `content`, **không** phải `StepVersion.parent_version` (field đó chỉ track lineage trong cùng 1 step, ví dụ script v2 phục hồi từ script v1). Đồng thời thêm 2 endpoint mới: `PUT .../versions/current` (sửa tay, insert-only) và `POST /projects/{id}/steps/{step}/approve` (hiện thực hoá lần đầu endpoint đã khai báo ở §2, scope task 4-5 giới hạn outline/script).
