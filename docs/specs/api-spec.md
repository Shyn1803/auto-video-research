...
| GET 🅞 | `/projects/{id}/steps/{step}/versions/{version}` | **(task 5-9, additive)** Chi tiết 1 version gồm cả `content` — dùng cho màn "Xem" (readonly overlay); `VersionOut` (list/current/restore) cố tình không mang `content` để payload list nhỏ, và `compare` chỉ trả diff/scene-diff (không phải content thô) cho 4 step nội dung chính, nên trước 5-9 không có cách nào xem lại content thô của 1 version cũ. 404 nếu version không tồn tại. Đăng ký **sau** route `compare` trong router (cùng 1 segment động `{version}` — nếu đăng ký trước sẽ "nuốt" mất request tới `.../versions/compare`, gây 422 do không parse được "compare" thành int) |
...
**Contract change note (task 5-9):** thêm `GET .../versions/{version}` (additive, không đổi shape của endpoint nào có sẵn) trả `VersionDetailOut` = `VersionOut` + `content: dict`, phục vụ màn "Xem". Đồng thời sửa lỗi cross-cutting phát hiện ở task 4-4 (`memory/project-memory.md` 2026-07-18): router này trước đó mount ở bare `/projects/...` trong khi frontend axios client luôn prefix `/api` — mọi request thật sự sẽ 404. Đã thêm `prefix="/api"` cho `app/api/versions.py` (khớp convention của admin routers); `projects.py`/`runs.py`/`scenes.py`/`claims.py`/`sources.py` vẫn còn lỗi này, cần task khác sửa.
