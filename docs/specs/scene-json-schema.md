...
**Nguồn schema duy nhất:** Pydantic model tại `backend/app/schemas/scene.py` → export JSON Schema → Zod tại `packages/remotion-templates/src/schema.ts` validate cùng file JSON Schema đó. **Không được định nghĩa tay ở 2 nơi.**
...
| `transition` | Transition | ✔ | | Transition **ra khỏi** scene này |

**Cache key:** `sha256(canonical_json(scene) + template_version + format)` — canonical = sort keys, bỏ field `scene_number` (đổi thứ tự không phá cache).
